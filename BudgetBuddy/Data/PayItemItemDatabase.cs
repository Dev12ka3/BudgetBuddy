...
namespace BudgetBuddy.Data
{
    public class PayItemItemDatabase
    {
        ...
        public async Task<List<PayItem>> GetItemsByCategoryAsync(string category)
        {
            try
            {
                return await Database.Table<PayItem>().Where(i => i.Category == category).ToListAsync();
            }
            catch (Exception)
            {
                return await ReturnEmptyList();
        ...
        public async Task<List<PayItem>> GetAllTransactionsAsync()
        {
            try
            {
                return await Database.Table<PayItem>().ToListAsync();
            }
            catch (Exception)
            {
                return await ReturnEmptyList();
